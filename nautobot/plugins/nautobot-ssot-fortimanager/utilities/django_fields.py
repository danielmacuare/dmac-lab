...
from nautobot_firewall_models.models.address import FQDN as model_name  # Assuming this is your model_name
...
table_concrete = Table(title="Django Model Fields (Concrete)", show_lines=True, header_style="bold magenta")
...
for field in model_name._meta.concrete_fields:
    ...
    table_concrete.add_row(field.name, type(field).__name__, is_relation_str, related_model_name)
...
table_local = Table(title="Django Model Fields (Local)", show_lines=True, header_style="bold magenta")
...
for field in model_name._meta.local_fields:
    ...
    table_local.add_row(field.name, type(field).__name__, is_relation_str, related_model_name)
