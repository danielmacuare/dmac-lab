...
console = Console()
rich_table = Table(
    title=f"{model_name} Model Fields and Relationships", show_lines=True, header_style="bold magenta"
)
...
for field in model._meta.get_fields():
    ...
    if field.is_relation:
        ...
        if hasattr(field, "field") and field.field:  # For reverse relations (e.g., ManyToOneRel)
            related_field_on_model = field.field.name

        rich_table.add_row(field_name, "Relation", relation_type, related_model_name, related_field_on_model)
    elif field.concrete:
